...
//            return $foo->error;
    ...
    static public function trataMail($param)
    {
        switch($param['tipo']) {
            case 'forgot':
                $mensagem  = '<a href="'. URL_PUBLIC .'/remember/'.$param['session'].'" target="_blank">Redefinir Senha</a>';
                $mensagem2  =  URL_PUBLIC .'/remember/'.$param['session'];
                $nome  =  $param['nome'];
                self::refefinir('Redefinir Senha',$mensagem,$mensagem2,$nome,[$param['email']]);
                break;
            case 'contato':
                $assunto = explode('#',$param['assunto']);
                $mensagem  = 'Nome: '.$param['nome'].'<br>';
                $mensagem  .= 'Email: '.$param['email'].'<br>';
                $mensagem  .= 'Telefone: '.$param['telefone'].'<br>';
                $mensagem  .= 'Mensagem: '.$param['mensagem'].'<br>';
                self::mail(current($assunto),$mensagem,[end($assunto)]);
                break;
        }
    }

    static public function refefinir($assunto, $msg, $msg2, $nome, $email = [], $cc = false, $anexo = false)
    {
      ...
      <html>
                      ...
                      <td class="wrapper">
                        <table role="presentation" border="0" cellpadding="0" cellspacing="0">
                          <tr>
                            ...
                            $body .= $nome;
                            $body .= '</h1>
                            <p>Você solicitou recentemente a redefinição da senha da sua conta do sistema <b>"'.APP_TITLE.'"</b>.<br> Use o botão abaixo para redefini-la.<br> Essa redefinição de senha é válida apenas pelas próximas 24 horas.</strong></p>
                              ...
                              <p>Se você não solicitou uma redefinição de senha, ignore este e-mail ou entre em contato com o suporte se tiver alguma dúvida.</p><br>
                              <p>Se você estiver tendo problemas com o botão acima, copie e cole o URL abaixo no seu navegador.</p>';
                              $body .= $msg2;
                              $body .= '<br><br>
                              <p>Obrigado,
                              <br>Equipe SMART Soluções Inteligentes.</p>
                            </td>
                          </tr>
                        </table>
                      </td>
        ...
        try {
            ...
            $mail->Host       = MAIL_HOST;                        // Defina o servidor SMTP para enviar através
            $mail->SMTPAuth   = MAIL_AUTH;                                   // Ativar autenticação SMTP
            $mail->Username   = MAIL_USER;                    // Nome de usuário SMTP
            $mail->Password   = MAIL_PASS;                            // Senha SMTP
            $mail->SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS;         // Ativar criptografia TLS; O `PHPMailer :: ENCRYPTION_SMTPS` também foi aceito
            $mail->Port       = MAIL_PORT;                                    // Porta TCP à qual se conectar
